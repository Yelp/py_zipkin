...
def test_add_sa_binary_annotation():
    ...
    with context:
        assert context.logging_context.sa_endpoints == []
        context.add_sa_binary_annotation(
            port=123,
            service_name='test_service',
            host='1.2.3.4',
        )
        expected_sa_endpoint = encoding.create_endpoint(
            port=123,
            service_name='test_service',
            host='1.2.3.4',
        )
        assert context.logging_context.sa_endpoints[0] == \
        ...
        with nested_context:
            ...
            expected_nested_sa_endpoint = encoding.create_endpoint(
                port=456,
                service_name='nested_service',
                host='5.6.7.8',
            )
            assert nested_context.sa_endpoints[0] == \
...
def test_adding_sa_binary_annotation_without_sampling():
    ...
    with context:
        ...
        assert context.sa_endpoints[0] == expected_sa_endpoint
